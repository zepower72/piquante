...
exports.signup = (req, res, next) => {
  if (!passwordVerify.validate(req.body.password)) {
    // Si le mot de passe ne correspond pas aux options de passwordVerify
    bcrypt // On crypte le mot de passe
      .hash(req.body.password, 10) // On hash le mot de passe avec 10 tours de cryptage
      .then((hash) => {
        // On crée un nouvel utilisateur avec le mot de passe crypté
        const user = new User({
  ...
  User.findOne({ email: req.body.email }) // On cherche l'utilisateur dans la base de données
    .then((user) => {
      ...
      bcrypt // On compare le mot de passe
        ...
        .then((valid) => {
          // On récupère le résultat de la comparaison
          if (!valid) {
